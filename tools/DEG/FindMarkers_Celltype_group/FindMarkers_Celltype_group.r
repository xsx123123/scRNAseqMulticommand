...
# findmarker deg rseult annotation module
...
FindMarkers_Celltype_group <- function(Seurat,Celltype = 'celltype',Pair = 'group',
                                       ...
                                       mus_gene_infor = "/titan3/local_zhang_jian/database/genome/mm10/Mus_musculus.gene_info",
                                       human_gene_infor = '/titan3/local_zhang_jian/database/genome/hg19/Homo_sapiens.gene_info'){
  # LOADING PACAKGES
  require(tidyverse)
  require(Seurat)
  require(crayon)
  ...
  if (!is.null(DEG)) {
    ...
    if (!is.null(filter_deg)) {
      filter_deg <- tryCatch({
        DEG_annotation(filter_deg, taxid, mus_gene_infor, human_gene_infor)
      }, error = function(e) {
